...
#pragma omp parallel for schedule(static)
  ...
  void PredictNoInsideCache(const SparseBatch::Inst &inst,
	  ...
	  DCHECK(thread_cache->size() == model.param.num_feature) << "Number of features are different:" << thread_cache->size() << " != " << model.param.num_feature << "(expected)";
	  DCHECK(out_preds.size() == model.param.num_output_group * (model.param.size_leaf_vector + 1)) << "Size are different.";
	  ntree_limit *= model.param.num_output_group;
	  if (ntree_limit == 0 || ntree_limit > model.trees.size()) {
		  ntree_limit = static_cast<unsigned>(model.trees.size());
	  }
	  ...
	  for (int gid = 0; gid < model.param.num_output_group; ++gid) {
		  out_preds[gid] =
			  PredValue(inst, model.trees, model.tree_info, gid, root_index,
				  thread_cache, 0, ntree_limit) + model.base_margin;
	  }
  }
  ...
  void PredictOutputSize(const SparseBatch::Inst &inst,
	                     ...
	                     unsigned ntree_limit,
	                     unsigned root_index) override {
	  len = model.param.num_output_group * (model.param.size_leaf_vector + 1);
	  out_size_buffer = 0;
  }
