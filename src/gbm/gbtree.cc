...
class GBTree : public GradientBooster {
 public:
  ...
  void Configure(const std::vector<std::pair<std::string, std::string> >& cfg) override {
    ...
    monitor.Init("GBTree", tparam.debug_verbose == 1);
  }
  ...
  void PredictNoInsideCache(const SparseBatch::Inst &inst,
    ...
    return predictor->PredictNoInsideCache(inst, out_preds, pred_buffer, pred_counter, model_, 
										   ntree_limit, root_index, void_thread_cache);
  }
  ...
  void PredictOutputSize(const SparseBatch::Inst &inst,
                         ...
                         unsigned ntree_limit,
                         unsigned root_index) override {
    return predictor->PredictOutputSize(inst, len, out_size_buffer, model_, ntree_limit, root_index);
  }
 ...
 protected:
  ...
  CommitModel(std::vector<std::unique_ptr<RegTree> >&& new_trees,
    ...
    predictor->UpdatePredictionCache(model_, &updaters, (int)new_trees.size());
  }
...
class Dart : public GBTree {
 ...
 protected:
  ...
  DCHECK(thread_cache->size() == model_.param.num_feature) << "Number of featuress are different.";
    ...
    DCHECK(out_preds.size() == model_.param.num_output_group * (model_.param.size_leaf_vector + 1)) << "Size are different.";
    ntree_limit *= model_.param.num_output_group;
    if (ntree_limit == 0 || ntree_limit > model_.trees.size()) {
      ntree_limit = static_cast<unsigned>(model_.trees.size());
    }
    ...
    for (int gid = 0; gid < model_.param.num_output_group; ++gid) {
      out_preds[gid]
        = PredValue(inst, gid, root_index,
            thread_cache, 0, ntree_limit) + model_.base_margin;
    }
  }
  ...
  void PredictOutputSize(const SparseBatch::Inst &inst,
                          ...
                          unsigned ntree_limit,
                          unsigned root_index) override {
    len = model_.param.num_output_group * (model_.param.size_leaf_vector + 1);
    out_size_buffer = 0;
  }
  ...
  inline size_t NormalizeTrees(size_t size_new_trees, std::vector<size_t> &idx_drop) {
    float lr = 1.0f * dparam.learning_rate / size_new_trees;
    ...
    if (num_drop == 0) {
      ...
      if (dparam.normalize_type == 1) {
        ...
        // normalize_type 0
        float factor = 1.0f * num_drop / (num_drop + lr);
        for (size_t i = 0; i < idx_drop.size(); ++i) {
          weight_drop[idx_drop[i]] *= factor;
        }
        for (size_t i = 0; i < size_new_trees; ++i) {
          weight_drop.push_back(1.0f / (num_drop + lr));
        }
      }
    }
    // reset
  ...
  inline void InitThreadTemp(int nthread) {
    int prev_thread_temp_size = (int)thread_temp.size();
